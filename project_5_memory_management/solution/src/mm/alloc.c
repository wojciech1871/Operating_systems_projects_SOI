...
PRIVATE int set_algorithm; /* if = 0 - deafult algorithm  if = 1 - worst fit algorithm */
 ...
 */
  ...
  register struct hole *biggest_hole, *prev_biggest; /* needed in algorithm worst fit */

  phys_clicks old_base;

  if(set_algorithm == DEFAULT_ALG) {
  do {
	hp = hole_head;
  ...
  if(set_algorithm == 1) {
  do {
  biggest_hole = hp = hole_head;
  while (hp != NIL_HOLE && hp->h_base < swap_base) {
    if (hp->h_len > biggest_hole->h_len) {
      /* We found a hole that is bigger */
      prev_biggest = prev_ptr;  /* remember hole preceding biggest hole */
      biggest_hole = hp;
    }
    prev_ptr = hp;
    hp = hp->h_next;
  }
  /* We found a hole that is the biggest. Can we use it? */
  if(biggest_hole->h_len >= clicks)
  {
    ...
    /* Delete the hole if used up completely. */
    if (biggest_hole->h_len == 0) del_slot(prev_biggest, biggest_hole);
...
{
  register struct hole *hp;
  phys_clicks sys_buffer[2*NR_HOLES+1];
  phys_clicks *user_buffer = (phys_clicks *)mm_in.m1_p1;
  size_t nbytes = mm_in.m1_i1;
  int who = mm_in.m_source;

  int num_pairs;
  int i;
  int k=0;

  num_pairs = nbytes/(2*sizeof(phys_clicks));
  if(nbytes%(2*sizeof(phys_clicks)) < sizeof(phys_clicks)) num_pairs -= 1;

  hp = hole_head;
  for(i=1; i<=num_pairs; i+=1)
  {
    sys_buffer[k] = hp->h_len;
    k+=1;
    sys_buffer[k] = hp->h_base;
    k+=1;
    hp=hp->h_next;
    if(hp == NIL_HOLE || hp->h_base >= swap_base) break;
  }
  sys_buffer[k] = 0;
  sys_copy(MM_PROC_NR, D, (phys_clicks)sys_buffer, who, D, (phys_clicks)user_buffer, (phys_bytes)nbytes);
  return(i);
}
