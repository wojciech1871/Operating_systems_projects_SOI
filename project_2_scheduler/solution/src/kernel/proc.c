 ...
 */
  ...
  if (rdy_head[USER_Q] == NIL_PROC)
	...
	else
	{
		...
		{
			rp->current_priority = rp->base_priority;
			rp->p_nextready = NIL_PROC;
 ...
 */
 printf("%d \n", rdy_head[USER_Q]->p_pid);
  ...
  if (rdy_head[USER_Q] == NIL_PROC) return;
	if(rdy_head[USER_Q]->group_proc != GROUP_C)
	{
		rdy_head[USER_Q]->current_priority -= 1;
		if(rdy_head[USER_Q]->current_priority <= 0)
		{
			rdy_head[USER_Q]->current_priority = rdy_head[USER_Q]->base_priority;
