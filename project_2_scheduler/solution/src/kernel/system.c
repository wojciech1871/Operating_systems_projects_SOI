...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->group_proc=rpp->group_proc;
  rpc->current_priority=rpp->current_priority;
  rpc->base_priority=rpp->base_priority;
...
{
	int i;
	for(i = NR_TASKS; i < NR_TASKS + NR_PROCS; i+=1)
		if(proc[i].p_pid == pid && proc[i].p_priority != PPRI_NONE)
			return &proc[i];
	return NIL_PROC;
}

PRIVATE int do_setpri(m_ptr)    /* our new function for SYS_CALL 22 */
message *m_ptr;
{
  struct proc *rp;
  int number;
  ...
  value = m_ptr->m1_i2;
  pid_num = m_ptr->m1_i3;
  rp=find_user_proc_by_pid(pid_num);
  ...
  switch(number)
  {
    ...
    case 4:
          if(rp != NIL_PROC)
          {
            rp->base_priority=value;
            return(OK);
          }
          break;
    case 5:
          if(rp != NIL_PROC && value >=0 && value <=3)
          {
            rp->group_proc=value;
            return(OK);
          }
...
PRIVATE int do_getpri(m_ptr)    /* our new function for SYS_CALL 23 */
message *m_ptr;
{
  struct proc *rp;
  int number;
